...
export const KanbanCard: React.FC<KanbanCardProps> = ({ 
  ...
  const handleDragOver = (e: React.DragEvent) => {
      e.preventDefault();
      e.stopPropagation(); // Important: Prevent bubbling to column
  ...
  const handleDrop = (e: React.DragEvent) => {
      e.preventDefault();
      e.stopPropagation(); // Important: Handle drop here, don't bubble to column
      setIsDragOver(false);
  ...
  if (isCompact) {
    return (
      <div 
        ...
        onDrop={handleDrop}
        className={`bg-white dark:bg-slate-700 px-3 py-2 rounded-lg shadow-sm border border-slate-200 dark:border-slate-600 hover:shadow-md hover:border-blue-300 transition-all cursor-grab active:cursor-grabbing flex items-center gap-3 group ${isDragOver ? 'border-t-4 border-t-blue-500' : ''}`}
  ...
  return (
    <div 
      ...
      onDrop={handleDrop}
      className={`
        p-2 lg:p-2.5 rounded-lg shadow-sm border transition-all cursor-grab active:cursor-grabbing group flex flex-col gap-1 relative
        ${cardStyleClass}
        ${!isMyJob && job.priority === Priority.NORMAL && !isDragOver && 'hover:shadow-md hover:border-blue-300'}
        active:rotate-1 active:scale-105 active:shadow-xl active:z-50
      `}
      onClick={() => onSelect(job)}
    ...
    >
      ...
      <div className="pointer-events-none">
        <h4 className="font-bold text-slate-800 dark:text-slate-100 text-sm lg:text-base leading-tight mb-0.5 truncate">{job.title}</h4>
        <p className="text-xs lg:text-sm text-slate-500 dark:text-slate-400 truncate">{job.clientName}</p>
      </div>
      
      <div className="flex items-center justify-between pt-1 border-t border-slate-50/50 dark:border-slate-600/50 text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 pointer-events-none">
        <div className="flex items-center gap-1.5 min-w-0">
          <div className={`w-4.5 h-4.5 rounded-full flex items-center justify-center shrink-0 ${isMyJob ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300' : 'bg-slate-100 dark:bg-slate-600 text-slate-400 dark:text-slate-300'}`}>
             {isMultiStaff ? <Users size={11} /> : <User size={11} />}
          </div>
          <span className={`text-[11px] font-medium truncate ${isMyJob ? 'text-blue-700 dark:text-blue-300 font-bold' : 'text-slate-600 dark:text-slate-400'}`} title={staffName}>
              {staffName}
          </span>
        </div>
        <div className={`flex items-center gap-1.5 bg-slate-50 dark:bg-slate-700 px-1.5 py-0.5 rounded shrink-0 ${daysRemaining <= 3 ? 'text-red-600 dark:text-red-400 font-bold' : ''}`}>
